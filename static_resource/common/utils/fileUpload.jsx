...
const FileUpload = createReactClass({
    ...
    IEUpload(e) {
        ...
        // IE里会诡异地调用多次 handleOnLoad， 所以加一个是否已经执行了 callbacked 的 flag
        ...
        function handleOnLoad() {
            ...
            try {
                let resp = that.IECallback(that.dataType, partIEID)
                that.setState({
                    IESuccessed: true
                })
                ...
                oInput.parentNode.replaceChild(oInput.cloneNode(true), oInput);
                callbacked = true
            }
        }
            ...
            render = (
                <div className={this.props.className} style={this.props.style}>
                    ...
                    <div onClick={this.commonChooseFile}
                        style={{overflow:'hidden',postion:'relative',display:this.wrapperDisplay,verticalAlign: 'top'}}
                    >
    ...
    _multiIEForm() {
        ...
        function _insertIEForm(formArr,i) {
            ...
            //EDIT file field name to use this.fileFieldName instead of `ajax_upload_hidden_input_${this.IETag}${i}`
            const fileInput =
                <input type="file" name={this.fileFieldName} id={`ajax_upload_hidden_input_${this.IETag}${i}`}
                    ref={`ajax_upload_hidden_input_${this.IETag}${i}`} onChange={this.IEChooseFile} onClick={this.IEBeforeChoose}
                    style={style} {...restAttrs}
                />

            // 自定义的参数
            const customParamInput = []
            for (var param in this.paramAddToField) {
                if (this.paramAddToField.hasOwnProperty(param)) {
                    customParamInput.push(
                        <input type='hidden' name={`${param}`} value={`${this.paramAddToField[param]}`} />
                    )
                }
            }
            ...
            formArr.push((
                <form id={`ajax_upload_file_form_${i}`} method="post" target={`ajax_upload_file_frame_${i}`}
                    ...
                    <div style={{
                        overflow:'hidden',
                        position:'relative',
                        display: this.state.IESuccessed ? 'block' : this.wrapperDisplay
                        }}>
                        {this.state.uploadBtn}
                        <input type="submit"
                            style={{
